  ...
  <el-card class="form-container" shadow="never" v-if="homeAdvertise">
    ...
    >
      ...
      <el-form-item label="是否显示：">
        <el-radio-group v-model="homeAdvertise.showStatus">
      ...
      <el-form-item label="是否显示在导航栏：">
        <el-radio-group v-model="homeAdvertise.navStatus">
          <el-radio :label="1">是</el-radio>
          <el-radio :label="0">否</el-radio>
        </el-radio-group>
      </el-form-item>
...
import { fetchCategorycreate,fetchCategoryUpdate,fetchCategory } from "@/api/healthCategory";
...
export default {
  ...
  created() {
    if (this.isEdit) {
      fetchCategory(this.$route.query.id).then(response => {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$confirm("是否提交数据", "提示", {
            ...
            if (this.isEdit) {
              fetchCategoryUpdate(this.$route.query.id, this.homeAdvertise).then(
                response => {
              ...
              this.homeAdvertise.parentId = this.$route.query.parentId;
              fetchCategorycreate(this.homeAdvertise).then(response => {
                this.$refs[formName].resetFields();
                this.homeAdvertise = Object.assign({}, defaultHomeAdvertise);
                this.$message({
                  message: "提交成功",
