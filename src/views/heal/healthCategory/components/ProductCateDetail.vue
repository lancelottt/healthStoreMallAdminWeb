...
<script>
import {fetchCategoryUpdate,fetchCategory,fetchCategorycreate } from '@/api/healthCategory'
import {
  fetchList,
  createProductCate,
  updateProductCate,
        ...
        if (valid) {
          this.$confirm("是否提交数据", "提示", {
            ...
            if (this.isEdit) {
              ...
              fetchCategoryUpdate(this.$route.query.id, this.productCate).then(
                response => {
                  this.$message({
                    message: "修改成功",
                    type: "success",
              ...
              this.productCate.productAttributeIdList = this.getProductAttributeIdList();
              fetchCategorycreate(this.productCate).then(response => {
