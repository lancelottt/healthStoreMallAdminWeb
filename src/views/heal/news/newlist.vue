    ...
    <div class="table-container">
      <el-table ref="flashTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column label="标题" align="center">
          <template slot-scope="scope">{{scope.row.informationName}}</template>
        </el-table-column>
        <el-table-column label="来源" width="140" align="center">
          <template slot-scope="scope">{{scope.row.informationSource}}</template>
        </el-table-column>
...
<script>
import { fetchNewsList,fetchNewsDelete } from "@/api/health";
...
export default {
  ...
  methods: {
      handleAdd() {
          this.$router.push({path: '/heal/newsAdd'})
      },
    handleResetSearch() {
    ...
    handleDelete(index, row) {
      this.$confirm("是否要删除该活动?", "提示", {
        ...
        fetchNewsDelete(row.id).then(response => {
          this.$message({
            type: "success",
            message: "删除成功!"
          });
