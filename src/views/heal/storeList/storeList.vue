    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="营业时间" align="center">
          <template slot-scope="scope">{{scope.row.operateTime}} {{scope.row.region}}</template>
        </el-table-column>
...
<script>
import { fetStoreList, fetStoreDelete } from "@/api/storeList";
...
export default {
  ...
  methods: {
    handleAdd() {
      this.$router.push({ path: "/heal/storeListadd" });
    },
    handleResetSearch() {
    ...
    handleDelete(index, row) {
      this.$confirm("是否要删除该活动?", "提示", {
        ...
        fetStoreDelete(row.id).then(response => {
          this.$message({
            type: "success",
            message: "删除成功!"
          });
    ...
    getList() {
      ...
      fetStoreList(this.listQuery).then(response => {
