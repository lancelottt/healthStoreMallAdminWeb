    ...
    <div class="table-container">
      <el-table ref="flashTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column label="描述" align="center">
          <template slot-scope="scope">{{scope.row.chargeSchemeDescribe}}</template>
        </el-table-column>
        <el-table-column label="内容" width="140" align="center">
          <template slot-scope="scope">{{scope.row.chargeSchemeName}}</template>
        </el-table-column>
         <el-table-column label="封面" width="140" align="center">
          <template slot-scope="scope">
            <img :src="scope.row.schemeImgUrl" width="50" height="50">
          </template>
        </el-table-column>
...
<script>
import { PayList,updateShowStatus ,addDicFlash ,upXiugaidateFlash} from "@/api/paylist";
...
//      navClassifyDicId.push(row.chargeSchemeCode)
        data.append('id',navClassifyDicId);
        data.append('hidden',row.hidden);
        data.append('labelIds','');
        data.append('chargeSchemeCode',row.chargeSchemeCode);
...
//  },
    handleDicUpdate(index, row) {
    	this.$router.push({path:'/pay/addupdate',query:{id:row.id,chargeSchemeCode:row.chargeSchemeCode}});
//      this.dialogVisible = true;
//      this.isEdit = true;
//      this.flashPromotion = Object.assign({},row);
      },
    ...
    getList() {
      ...
      PayList(this.listQuery).then(response => {
